...
Labeled questions: **14** (1 gap / 13 not)
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| words · pages with overlap>=0.6 is <= 0 | **0.20** | 1.00 | 0.33 | 0.71 | 1 | 4 | 0 | 9 |
| words · highest overlap < 0.6 | **0.20** | 1.00 | 0.33 | 0.71 | 1 | 4 | 0 | 9 |
| words · pages with overlap>=0.7 is <= 0 | **0.17** | 1.00 | 0.29 | 0.64 | 1 | 5 | 0 | 8 |
| words · highest overlap < 0.7 | **0.17** | 1.00 | 0.29 | 0.64 | 1 | 5 | 0 | 8 |
| words · pages with overlap>=0.6 is <= 1 | **0.14** | 1.00 | 0.25 | 0.57 | 1 | 6 | 0 | 7 |
| words · pages with overlap>=0.7 is <= 1 | **0.12** | 1.00 | 0.22 | 0.50 | 1 | 7 | 0 | 6 |
| stems · pages with overlap>=0.8 is <= 0 | **0.12** | 1.00 | 0.22 | 0.50 | 1 | 7 | 0 | 6 |
| stems · highest overlap < 0.8 | **0.12** | 1.00 | 0.22 | 0.50 | 1 | 7 | 0 | 6 |

**Highest F1:** words · pages with overlap>=0.6 is <= 0 (F1 0.33, precision 0.20)
...
|---|---:|---:|---|
| `words` | 0.33 | 0.20 | words · pages with overlap>=0.6 is <= 0 |
| `stems` | 0.22 | 0.12 | stems · pages with overlap>=0.8 is <= 0 |
| `synonyms` | 0.22 | 0.12 | synonyms · pages with overlap>=0.8 is <= 0 |
| `embeddings` | 0.18 | 0.10 | embeddings · pages with overlap>=0.7 is <= 0 |

## Page count vs highest overlap

Phase 0 observed that highest overlap is the wrong metric. Measured:

- Page-count family — best F1: **0.33** (words · pages with overlap>=0.6 is <= 0)
- Highest-overlap family — best F1: **0.33** (words · highest overlap < 0.6)
